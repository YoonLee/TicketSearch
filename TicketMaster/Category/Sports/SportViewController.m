...
- (void)viewDidLoad
{
    ...
    self.sportTV = [[UITableView alloc] initWithFrame:CGRectZero style:UITableViewStylePlain];
    [self.sportTV setDelegate:self];
    [self.sportTV setDataSource:self];
    [self.sportTV setRowHeight:60];
    [self.view addSubview:self.sportTV];
    
    QueryManager *qm = [[[QueryManager alloc] init] autorelease];
    [qm shakeEQ:@"nba&per_page=100"];
    [qm setDelegate:self];
    [qm executeQuery];
}
...
{
    static NSString *CellIdentifier = @"SportCellIdentifier";
